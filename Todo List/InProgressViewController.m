...
-(void)viewWillAppear:(BOOL)animated{
    ...
    NSUserDefaults *taskUserdefault=[NSUserDefaults standardUserDefaults];
      ...
      inProgressArr=[NSMutableArray new];
    ...
    //inprogress arr
      for (NSUInteger i=0;i<tasksArr.count;i++) {
          ...
          if ([[dic objectForKey:@"status"] isEqual:@"InProgress"]) {
              [inProgressArr addObject:dic];

      }
      }
    /// high arr
    for (NSUInteger i=0;i<inProgressArr.count;i++) {
        NSDictionary *dic=[inProgressArr objectAtIndex:i];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (isSorted==NO) {
       ...
       dict=[inProgressArr objectAtIndex:indexPath.row];
       ...
       if ([[dict objectForKey:@"priority"] isEqual:@"high"]) {
        ...
        printf("%lu", (unsigned long)inProgressArr.count);
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    InProgressDetailsViewController *taskDetails=[self.storyboard instantiateViewControllerWithIdentifier:@"InProgressDetailsViewController"];
    [taskDetails setTaskdicInprogress:[inProgressArr objectAtIndex:indexPath.row]];

    [taskDetails setIndexInprogress:indexPath.row];
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [_tableProgress deleteRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationFade];
    }else{
